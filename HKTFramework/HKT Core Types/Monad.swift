...
extension Construct where ConstructorTag:  MonadTag {
    public static func bind<B>(_ m: Construct<ConstructorTag, TypeParameter>) -> (@escaping (TypeParameter) -> Construct<ConstructorTag, B>) -> Construct<ConstructorTag, B> {
        return { continuation in
            return ConstructorTag.bind(m)(continuation)
        }
    }
    
    public static func pure<A>(_ value: A) -> Construct<ConstructorTag, A> {
        return ConstructorTag.pure(value)
    }
    
}

public func >>>=<A,B,M>(left: Construct<M, A>, right: @escaping (A) -> Construct<M,B>) -> Construct<M,B>
    where M: MonadTag
{
